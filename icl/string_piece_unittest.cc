...
TEST(CommonStringPieceTest, CheckComparisonOperators) {
#define CMP_Y(op, x, y)                                                       \
  {                                                                           \
    std::string lhs(x);                                                       \
    std::string rhs(y);                                                       \
    ASSERT_TRUE((StringPiece((lhs.c_str())) op StringPiece((rhs.c_str()))));  \
    ASSERT_TRUE((StringPiece((lhs.c_str())).compare(                          \
                     StringPiece((rhs.c_str()))) op 0));                      \
  }
...
#define CMP_N(op, x, y)                                                        \
  {                                                                            \
    std::string lhs(x);                                                        \
    std::string rhs(y);                                                        \
    ASSERT_FALSE((StringPiece((lhs.c_str())) op StringPiece((rhs.c_str()))));  \
    ASSERT_FALSE((StringPiece((lhs.c_str())).compare(                          \
                      StringPiece((rhs.c_str()))) op 0));                      \
  }
...
TEST(CommonStringPieceTest, CheckSTL) {
  std::string alphabet("abcdefghijklmnopqrstuvwxyz");
  std::string abc("abc");
  std::string xyz("xyz");
  std::string foobar("foobar");

  StringPiece a(alphabet);
  StringPiece b(abc);
  StringPiece c(xyz);
...
TEST(CommonStringPieceTest, CheckFind) {
  std::string alphabet("abcdefghijklmnopqrstuvwxyz");
  std::string abc("abc");
  std::string xyz("xyz");
  std::string foobar("foobar");
  ...
  std::string not_found("xx not found bb");
  StringPiece g(not_found);
  ...
  std::string one_two_three_four("one,two:three;four");
  std::string comma_colon(",:");
  ASSERT_EQ(3U, StringPiece(one_two_three_four).find_first_of(comma_colon));
  ...
  std::string fifty_six("56");
  StringPiece i(fifty_six);
...
TEST(CommonStringPieceTest, CheckCustom) {
  std::string foobar("foobar");
  ...
  // as_string
  std::string s3(a.as_string().c_str(), 7);  // Note, has an embedded NULL
  ASSERT_TRUE(c == s3);
...
TEST(CommonStringPieceTest, CheckNULL) {
...
TEST(CommonStringPieceTest, CheckComparisons2) {
  std::string alphabet("abcdefghijklmnopqrstuvwxyz");
  std::string alphabet_z("abcdefghijklmnopqrstuvwxyzz");
  std::string alphabet_y("abcdefghijklmnopqrstuvwxyy");
  StringPiece abc(alphabet);

  // check comparison operations on strings longer than 4 bytes.
  ASSERT_TRUE(abc == StringPiece(alphabet));
  ASSERT_EQ(abc.compare(StringPiece(alphabet)), 0);

  ASSERT_TRUE(abc < StringPiece(alphabet_z));
  ASSERT_LT(abc.compare(StringPiece(alphabet_z)), 0);

  ASSERT_TRUE(abc > StringPiece(alphabet_y));
  ASSERT_GT(abc.compare(StringPiece(alphabet_y)), 0);
}
...
TEST(CommonStringPieceTest, StringCompareNotAmbiguous) {
  ASSERT_TRUE(std::string("hello").c_str() == std::string("hello"));
  ASSERT_TRUE(std::string("hello").c_str() < std::string("world"));
}

TEST(CommonStringPieceTest, HeterogenousStringPieceEquals) {
  std::string hello("hello");

  ASSERT_TRUE(StringPiece(hello) == hello);
  ASSERT_TRUE(hello.c_str() == StringPiece(hello));
}

TEST(CommonStringPieceTest, CheckConstructors) {
  std::string str("hello world");
  std::string empty;

  ASSERT_TRUE(str == StringPiece(str));
  ASSERT_TRUE(str == StringPiece(str.c_str()));
  ASSERT_TRUE(std::string("hello") == StringPiece(str.c_str(), 5));
  ASSERT_TRUE(empty == StringPiece(str.c_str(),
      static_cast<typename StringPiece::size_type>(0)));
  ASSERT_TRUE(empty == StringPiece(NULL));
  ASSERT_TRUE(empty == StringPiece(NULL,
      static_cast<typename StringPiece::size_type>(0)));
  ASSERT_TRUE(empty == StringPiece());
  ASSERT_TRUE(str == StringPiece(str.begin(), str.end()));
  ASSERT_TRUE(empty == StringPiece(str.begin(), str.begin()));
  ASSERT_TRUE(empty == StringPiece(empty));
  ASSERT_TRUE(empty == StringPiece(empty.begin(), empty.end()));
}
